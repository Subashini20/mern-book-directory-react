            ...
            if (this.state.role === "user")
             {
                try
                 {
                     var response = await axios.post('https://mern-book-directory-node.herokuapp.com/register/signup',{
            ...
            if (this.state.role === "admin")
             {
                try 
                {
                     response = await axios.post('https://mern-book-directory-node.herokuapp.com/register/Adminsignup', {
                        username: this.state.username,
                        password: this.state.password,
                        email: this.state.email,
                        name: this.state.name,
                        ...
                        <div className="auth-inner" style={{ padding: "1%" }}>
                            <form onSubmit={(e) => handleSubmit(e)}>
                                ...
                                <div className="form-group">
                                    ...
                                    <input type="text" className="form-control" placeholder="please enter Chennai or Hyderabad" minLength={5} maxLength={50} required value={this.state.address} onChange={(e) => this.setState({ address: e.target.value })} />
                                </div><br></br>
                                ...
                                <div className="form-group">
                                    ...
                                    <input type="password" className="form-control" placeholder="Enter characters minimum 8 and maximum 10" minLength={8} maxLength={10} required value={this.state.password} onChange={(e) => this.setState({ password: e.target.value })} />
                                </div><br></br>
